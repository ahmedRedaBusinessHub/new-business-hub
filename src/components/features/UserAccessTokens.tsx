...
interface UserAccessTokensProps {
  userId: number;
}
...
export function UserAccessTokens({ userId }: UserAccessTokensProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Access Tokens ({total})</CardTitle>
      </CardHeader>
                ...
                {accessTokens.map((token: any) => (
                  <TableRow key={token.id}>
                    <TableCell className="font-mono text-xs">
                      {token.app_token?.substring(0, 20) + "..." || "-"}
                    </TableCell>
                    <TableCell>
                      {token.platform || "-"}
                    </TableCell>
                    <TableCell>
                      {token.auth_at
                        ? new Date(token.auth_at).toLocaleString()
                        : "-"}
                    </TableCell>
                    <TableCell>
                      {token.auth_expire_at
                        ? new Date(token.auth_expire_at).toLocaleString()
                        : "-"}
                    </TableCell>
                    <TableCell>
                      {token.created_at
