...
export async function GET(
  ...
  try {
    ...
    const res = await apiGet(`/user-access-tokens/user/${userId}`, {
      requireAuth: true,
    });
    ...
    if (search) {
      ...
      allData = allData.filter((token: any) => {
        ...
        return tokenStr.includes(query) || expiresAt.includes(query) || createdAt.includes(query) || platform.includes(query);
      });
    }
