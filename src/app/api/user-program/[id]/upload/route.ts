...
export async function POST(
  ...
  try {
    ...
    const refColumn = (formData.get("refColumn") as string) || "upload_documents";
    ...
    // Forward file names if provided
    if (fileNames.length > 0) {
      fileNames.forEach((name) => {
        backendFormData.append("fileNames", name);
      });
    }
    
    backendFormData.append("refColumn", refColumn);

    const response = await fetch(`${apiUrl}/user-program/${id}/upload`, {
      method: "POST",
      headers: {
